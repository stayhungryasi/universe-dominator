...
def collect(cfg):
    ...
    rows = []
    for s in fc.interleave_by_host(cfg["sources"]):
        text, outcome, code = fc.fetch(s["url"], s["name"], "signals", headers=UA)
        if text is None:
            print(f"[신호] {s['name']} 수집 실패 → 건너뜀 ({outcome} {code})", file=sys.stderr)
            continue
        try:
